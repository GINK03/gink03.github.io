...
### manifest.json の活用
 ...
 - データウェアハウスのセマンティックレイヤー（どのテーブルに何のカラムがあり、どういう意味か）の情報量が集約されている
 - **LLM へのコンテキスト注入**：`manifest.json`（または必要なモデルの抜粋）を LLM のプロンプトに渡すと、スキーマ知識を持った状態で自然言語からの SQL クエリ生成が可能になる
