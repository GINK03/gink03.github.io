...
## 充電関連
 ...
 - トラブルシューティング
   - ライトニングケーブルの4番目のピンが黒くなり充電できない
     - 原因
       - 充電用に大電流を流すピンであり、湿気や汗等でかんたんに錆びる
     - 対応
       - 錆びるような環境で使用しない
       - 接点復活剤を使用する
       - 錆をこそぎ落とす
     - 参考
       - [【iPhone充電ケーブル】Lightning端子が腐食する原因と対処法](https://blog-lounge.com/iphone-lightning/)

## 外部キーボード
 - 概要
   - iphoneで外部キーボードを有線か無線で接続することができる
 - 制限と対応
   - iOSはキーボードのショートカットの一部が使えない
     - e.g. `cmd + tab`でアプリを切り替えることができない
     - 代わりに`cmd + space`でspotlightを開いてアプリを切り替える
 - CapsLockの挙動
   - CapsLockを押すと、キーボードの状態(日本語と英語など)が変わる
   - `ctrl + space`で状態を切り替えるので無効化しても問題ない

## プライバシーレポート
 - 概要
   - どのアプリがどの程度のデータを収集しているかを可視化する
     - データの種類
       - 位置情報
       - 連絡先
       - アクセス先(ホスト別)
