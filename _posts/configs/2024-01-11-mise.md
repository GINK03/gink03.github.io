...
`mise x -- <command>` は `mise exec -- <command>` の短縮形で、普段はこちらを使うことが多い

意味としては、現在のシェルを恒久的に変更せず、そのコマンド実行プロセスだけにmiseの環境をセットして実行する

有効になるもの

 - `.mise.toml` / `mise.toml` の `[tools]`
 - mise管理ツールが優先されるように構築されたPATH（gitleaks, terraform, python等）
 - `[env]` の環境変数
...
| pre-commit hookなどmise環境を引き継がない場面 | `mise x -- command` |
| CI/CD（mise自体は入っている） | `mise x -- command` |
| シェルスクリプト内で確実性を上げたい | `mise x -- command` |
...
似た選択肢
 ...
 - `mise exec --` / `mise x --` — 単発コマンドだけに注入
 - `mise run <task>` — `.mise.toml` で定義したタスク実行（env自動）

```console
$ mise x -- gcloud config list
$ mise x -- terraform plan
$ mise x -- bash -c 'echo $GOOGLE_CLOUD_PROJECT'
```
