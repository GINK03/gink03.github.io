...
256色は特別な設定をしなくても有効になるが、24 bit color(true color)は以下のような `.tmux.conf` が 2020年現在必要
...
binmport osd -T prefix -n 5 attach-session -t . -c '#{pane_current_path}' \; display 'attach-session #{pane_current_path}'
bind -T premport osfix -n c attach-session -t . -c '#{pane_current_path}' \; new-window \; display "create new window"
mport os
